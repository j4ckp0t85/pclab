...
class Paginator{
	...
	public function __construct($total=0,$mid_range=7,$ipp_array=array(10,25,50,100,"All")) {
		...
		if($this->num_pages > 10) {
			$this->return = ($this->current_page > 1 And $this->total_items >= 10) ? "<a class=\"paginate\" href=\"$_SERVER[PHP_SELF]?page=".($this->current_page-1)."&ipp=$this->items_per_page$this->querystring\"><<</a> ":"<span class=\"inactive\" href=\"#\"><<</span> ";
			...
			$this->return .= (($this->current_page < $this->num_pages And $this->total_items >= 10) And ($this->items_per_page != "All") And $this->current_page > 0) ? "<a class=\"paginate\" href=\"$_SERVER[PHP_SELF]?page=".($this->current_page+1)."&ipp=$this->items_per_page$this->querystring\">>></a>\n":"<span class=\"inactive\" href=\"#\">>></span>\n";
	...
	public function display_items_per_page() {
		...
		return "<span class=\"paginate\">Record per pagina:</span><select class=\"paginate\" onchange=\"window.location='$_SERVER[PHP_SELF]?page=1&amp;ipp='+this[this.selectedIndex].value+'$this->querystring';return false\">$items</select>\n";
	}
	public function display_jump_menu() {
		...
		return "<span class=\"paginate\">Pagina:</span><select class=\"paginate\" onchange=\"window.location='$_SERVER[PHP_SELF]?page='+this[this.selectedIndex].value+'&amp;ipp=$this->items_per_page$this->querystring';return false\">$option</select>\n";
	}
